...
import com.androidsdk.snaphy.snaphyandroidsdk.models.AmazonPdf;
...
public class AmazonPdfDb{
  ...
  public AmazonPdfDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final AmazonPdf modelData) {
    ...
    public ContentValues getContentValues(AmazonPdf modelData){
    ...
    // Getting single c
    public   AmazonPdf get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("AmazonPdf", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    //db.close(); // Closing database connection
                    if (hashMap != null) {
                        AmazonPdfRepository repo = restAdapter.createRepository(AmazonPdfRepository.class);
                        repo.addStorage(context);
                        return (AmazonPdf)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    public void upsert__db(String id, AmazonPdf model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<AmazonPdf>  getAll__db() {
        ...
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<AmazonPdf>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    AmazonPdfRepository repo = restAdapter.createRepository(AmazonPdfRepository.class);
                    repo.addStorage(context);
                    modelList.add((AmazonPdf)repo.createObject(hashMap));
                }
    ...
    public DataList<AmazonPdf>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    public DataList<AmazonPdf>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<AmazonPdf> modelList = new DataList<AmazonPdf>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM AmazonPdf " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM AmazonPdf " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<AmazonPdf>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    // Getting All Data where
    public DataList<AmazonPdf>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<AmazonPdf> modelList = new DataList<AmazonPdf>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `AmazonPdf` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
        String countQuery = "SELECT  * FROM `AmazonPdf` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int count = cursor.getCount();
        cursor.close();
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final AmazonPdf modelData ){
      new Thread(new Runnable(){
        @Override
        public void run(){
          SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
    ...
    public void update__db(final String id,   final AmazonPdf modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
