...
public class CommentDetailDb{
  ...
  public CommentDetailDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final CommentDetail modelData) {
    ...
    public ContentValues getContentValues(CommentDetail modelData){
    ...
    // Getting single c
    public   CommentDetail get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("CommentDetail", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    //db.close(); // Closing database connection
                    if (hashMap != null) {
                        CommentDetailRepository repo = restAdapter.createRepository(CommentDetailRepository.class);
                        repo.addStorage(context);
                        return (CommentDetail)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(5) != null){
                          commentIdData = cursor.getString(5);
    ...
    public void upsert__db(String id, CommentDetail model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<CommentDetail>  getAll__db() {
        ...
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<CommentDetail>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    CommentDetailRepository repo = restAdapter.createRepository(CommentDetailRepository.class);
                    repo.addStorage(context);
                    modelList.add((CommentDetail)repo.createObject(hashMap));
                }
    ...
    public DataList<CommentDetail>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    public DataList<CommentDetail>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<CommentDetail> modelList = new DataList<CommentDetail>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM CommentDetail " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM CommentDetail " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<CommentDetail>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    // Getting All Data where
    public DataList<CommentDetail>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<CommentDetail> modelList = new DataList<CommentDetail>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `CommentDetail` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
        String countQuery = "SELECT  * FROM `CommentDetail` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int count = cursor.getCount();
        cursor.close();
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final CommentDetail modelData ){
      new Thread(new Runnable(){
        @Override
        public void run(){
          SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
    ...
    public void update__db(final String id,   final CommentDetail modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
