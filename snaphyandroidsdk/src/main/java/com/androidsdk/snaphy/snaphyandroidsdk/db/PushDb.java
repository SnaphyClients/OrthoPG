...
public class PushDb{
  ...
  public PushDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final Push modelData) {
    ...
    public ContentValues getContentValues(Push modelData){
    ...
    public void update__db(final String id,   final Push modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
