...
public class NewsDb{
  ...
  public NewsDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final News modelData) {
    ...
    public ContentValues getContentValues(News modelData){
    ...
    public DataList<News>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<News> modelList = new DataList<News>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM News " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM News " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<News>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    public void update__db(final String id,   final News modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
