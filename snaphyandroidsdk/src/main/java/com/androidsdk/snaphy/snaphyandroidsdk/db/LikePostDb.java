...
public class LikePostDb{
  ...
  public LikePostDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final LikePost modelData) {
    ...
    public ContentValues getContentValues(LikePost modelData){
    ...
    // Getting single c
    public   LikePost get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("LikePost", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    //db.close(); // Closing database connection
                    if (hashMap != null) {
                        LikePostRepository repo = restAdapter.createRepository(LikePostRepository.class);
                        repo.addStorage(context);
                        return (LikePost)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    public void upsert__db(String id, LikePost model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<LikePost>  getAll__db() {
        ...
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<LikePost>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    LikePostRepository repo = restAdapter.createRepository(LikePostRepository.class);
                    repo.addStorage(context);
                    modelList.add((LikePost)repo.createObject(hashMap));
                }
    ...
    public DataList<LikePost>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    public DataList<LikePost>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<LikePost> modelList = new DataList<LikePost>();
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM LikePost " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM LikePost " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<LikePost>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    // Getting All Data where
    public DataList<LikePost>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<LikePost> modelList = new DataList<LikePost>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `LikePost` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
        String countQuery = "SELECT  * FROM `LikePost` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int count = cursor.getCount();
        cursor.close();
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final LikePost modelData ){
      new Thread(new Runnable(){
        @Override
        public void run(){
          SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
    ...
    public void update__db(final String id,   final LikePost modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
